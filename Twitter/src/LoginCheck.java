...
public class LoginCheck extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			Connection con= DriverManager.getConnection("jdbc:mysql://localhost:3306/test", "root", ""); //Put d
			if (con != null) {
			...
			if(rs.next()) {
				...
				session.setAttribute("about", rs.getString(6));
				session.setAttribute("uname", uname);
			   response.sendRedirect("http://localhost:8080/Twitter/profile.jsp");
			}else {
				JOptionPane.showMessageDialog(dialog, "Usernaem and password don't match");
